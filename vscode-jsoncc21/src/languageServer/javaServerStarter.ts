...
const JSONCC_SERVER_NAME = 'com.jsoncc.ls-uber.jar';
const JSONCC_SERVER_MAIN_CLASS = 'com.jsoncc.ls.ls.JSONCCServerLauncher';
...
function prepareParams(microprofileJavaExtensions: string[]): string[] {
  ...
  const vmargs = workspace.getConfiguration("jsoncc").get("server.vmargs", '');
  ...
  const microprofileServerFound: Array<string> = glob.sync(`**/${JSONCC_SERVER_NAME}`, { cwd: serverHome });
  if (microprofileServerFound.length) {
    ...
    params.push(JSONCC_SERVER_MAIN_CLASS);
  } else {
